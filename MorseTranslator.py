...
def menu():
     ...
     print("[2] Translate English Word/s to Morse Code")
     print("[0] Exit Program")

menu()
option = int(input("Enter an action: "))

while option != 0:
     if option == 1:
          morse_string = input("Please enter a valid morse code (Separate words using '/'): ")
          print(morse_string + "transltaed to word/s means:")
          print(decode_morse(morse_fsm, morse_string))
          print(input("Press any key to continue"))

     elif option == 2:
          encoded = ''
          user_string = input("Please enter a message to be encoded in morse: ").upper()
          for character in user_string:
               tmp = []
               char_to_morse(morse_fsm, character, tmp)
               encoded_tmp = "".join(tmp)
               encoded = encoded + encoded_tmp + " "
          print(user_string + "will be translated to morse code as:")
          print(encoded)
          print("The conded message will be played in morse code please wait")
          morse_beep(encoded)
          print(input("Press any key to continue"))
     else:
...
print("Thank you for using the program. babye")
